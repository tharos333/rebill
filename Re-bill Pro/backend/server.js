...
.login-btn{width:100%;background:var(--accent);color:#fff;border:none;border-radius:10px;padding:13px;font-size:15px;font-family:var(--sans);font-weight:600;cursor:pointer;margin-top:8px;transition:background 0.15s}
...
.sidebar{width:220px;flex-shrink:0;background:var(--surface);border-right:1px solid var(--border);display:flex;flex-direction:column;padding:20px 0;position:fixed;height:100vh;z-index:10;overflow-y:auto}
.logo{padding:0 16px 20px}
.nav-section{font-size:10px;color:var(--muted);font-family:var(--mono);letter-spacing:1px;text-transform:uppercase;padding:10px 24px 4px}
...
#toast{position:fixed;bottom:20px;right:20px;background:var(--surface2);border:1px solid var(--border2);border-radius:10px;padding:10px 15px;font-size:13px;font-family:var(--mono);z-index:9999;transform:translateY(80px);opacity:0;transition:all 0.25s cubic-bezier(.16,1,.3,1)}
...
.activity-dot.payment{background:var(--green)}
.activity-dot.failed{background:var(--red)}
.activity-dot.charge{background:var(--accent)}
.activity-dot.dunning{background:var(--amber)}
.activity-dot.resume{background:var(--blue)}
.activity-dot.retry{background:var(--green)}
.activity-dot.proration{background:var(--accent)}
...
<div class="login-screen" id="login-screen">
  <div class="login-box">
    <div class="login-logo">
      <img id="login-logo" src="https://i.ibb.co/RTmtK8sm/subloop.png" alt="Subloop" style="width:170px;height:auto;display:block;margin:0 auto 10px;">
    </div>
    ...
    <div id="login-step-1">
      ...
      <div id="lockout-msg" style="display:none;background:var(--amber-bg);border:1px solid var(--amber);color:var(--amber);border-radius:8px;padding:10px 14px;font-size:12px;font-family:var(--mono);margin-top:12px;text-align:center;"></div>
    </div>
    <!-- Step 2: 2FA Code -->
    <div id="login-step-2" style="display:none;"></div>
  </div>
<!-- Premium 2FA Screen -->
<div id="twofa-screen" style="display:none;position:fixed;inset:0;background:var(--bg);z-index:1001;display:none;align-items:flex-start;justify-content:center;padding-top:10vh;">
  <div style="position:absolute;inset:0;background:radial-gradient(ellipse 600px 400px at 50% 38%,rgba(108,99,255,0.08) 0%,transparent 70%);pointer-events:none;"></div>
  <div style="width:420px;max-width:95vw;background:var(--surface);border:1px solid var(--border2);border-radius:20px;padding:40px 36px;position:relative;box-shadow:0 0 0 1px rgba(108,99,255,0.12),0 24px 64px rgba(0,0,0,0.4);">
    <!-- Purple glow -->
    <div style="position:absolute;top:-60px;left:50%;transform:translateX(-50%);width:200px;height:120px;background:radial-gradient(ellipse,rgba(108,99,255,0.25) 0%,transparent 70%);pointer-events:none;"></div>
    <!-- Logo -->
    <div style="text-align:center;margin-bottom:28px;">
      <img id="twofa-logo" src="https://i.ibb.co/RTmtK8sm/subloop.png" alt="Subloop" style="width:130px;height:auto;display:block;margin:0 auto;">
    </div>
    <!-- Shield icon -->
    <div style="text-align:center;margin-bottom:20px;">
      <div style="width:60px;height:60px;border-radius:16px;background:var(--accent-glow);border:1px solid rgba(108,99,255,0.3);display:flex;align-items:center;justify-content:center;margin:0 auto 16px;">
        <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" stroke-width="1.6"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><path d="M9 12l2 2 4-4" stroke="var(--accent)" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg>
      </div>
      <div style="font-size:18px;font-weight:700;letter-spacing:-0.3px;margin-bottom:6px;">Two-Factor Authentication</div>
      <div style="font-size:13px;color:var(--muted);font-family:var(--mono);line-height:1.5;">Enter the 6-digit code from your<br>authenticator app to continue</div>
    </div>
    <!-- OTP Input boxes -->
    <div style="display:flex;gap:8px;justify-content:center;margin-bottom:24px;" id="otp-boxes">
      <input type="text" maxlength="1" class="otp-box" id="otp-0" oninput="otpInput(this,0)" onkeydown="otpKey(event,0)" style="width:52px;height:56px;text-align:center;font-size:22px;font-weight:700;font-family:var(--mono);background:var(--surface2);border:1.5px solid var(--border2);border-radius:10px;color:var(--text);outline:none;transition:border-color 0.2s;" onfocus="this.style.borderColor='var(--accent)'" onblur="this.style.borderColor='var(--border2)'">
      <input type="text" maxlength="1" class="otp-box" id="otp-1" oninput="otpInput(this,1)" onkeydown="otpKey(event,1)" style="width:52px;height:56px;text-align:center;font-size:22px;font-weight:700;font-family:var(--mono);background:var(--surface2);border:1.5px solid var(--border2);border-radius:10px;color:var(--text);outline:none;transition:border-color 0.2s;" onfocus="this.style.borderColor='var(--accent)'" onblur="this.style.borderColor='var(--border2)'">
      <input type="text" maxlength="1" class="otp-box" id="otp-2" oninput="otpInput(this,2)" onkeydown="otpKey(event,2)" style="width:52px;height:56px;text-align:center;font-size:22px;font-weight:700;font-family:var(--mono);background:var(--surface2);border:1.5px solid var(--border2);border-radius:10px;color:var(--text);outline:none;transition:border-color 0.2s;" onfocus="this.style.borderColor='var(--accent)'" onblur="this.style.borderColor='var(--border2)'">
      <input type="text" maxlength="1" class="otp-box" id="otp-3" oninput="otpInput(this,3)" onkeydown="otpKey(event,3)" style="width:52px;height:56px;text-align:center;font-size:22px;font-weight:700;font-family:var(--mono);background:var(--surface2);border:1.5px solid var(--border2);border-radius:10px;color:var(--text);outline:none;transition:border-color 0.2s;" onfocus="this.style.borderColor='var(--accent)'" onblur="this.style.borderColor='var(--border2)'">
      <input type="text" maxlength="1" class="otp-box" id="otp-4" oninput="otpInput(this,4)" onkeydown="otpKey(event,4)" style="width:52px;height:56px;text-align:center;font-size:22px;font-weight:700;font-family:var(--mono);background:var(--surface2);border:1.5px solid var(--border2);border-radius:10px;color:var(--text);outline:none;transition:border-color 0.2s;" onfocus="this.style.borderColor='var(--accent)'" onblur="this.style.borderColor='var(--border2)'">
      <input type="text" maxlength="1" class="otp-box" id="otp-5" oninput="otpInput(this,5)" onkeydown="otpKey(event,5)" style="width:52px;height:56px;text-align:center;font-size:22px;font-weight:700;font-family:var(--mono);background:var(--surface2);border:1.5px solid var(--border2);border-radius:10px;color:var(--text);outline:none;transition:border-color 0.2s;" onfocus="this.style.borderColor='var(--accent)'" onblur="this.style.borderColor='var(--border2)'">
    </div>
    <!-- Error -->
    <div id="twofa-error" style="display:none;background:var(--red-bg);border:1px solid var(--red);color:var(--red);border-radius:8px;padding:10px 14px;font-size:12px;font-family:var(--mono);margin-bottom:16px;text-align:center;">Invalid code. Please try again.</div>
    <!-- Verify button -->
    <button onclick="verify2FA()" style="width:100%;background:var(--accent);color:#fff;border:none;border-radius:11px;padding:14px;font-size:15px;font-family:var(--sans);font-weight:600;cursor:pointer;transition:background 0.15s;margin-bottom:14px;" onmouseover="this.style.background='#7c74ff'" onmouseout="this.style.background='var(--accent)'">Verify & Sign in</button>
    <!-- Back link -->
    <div style="text-align:center;">
      <button onclick="backToLogin()" style="background:none;border:none;color:var(--muted);font-size:12px;cursor:pointer;font-family:var(--mono);transition:color 0.15s;" onmouseover="this.style.color='var(--text)'" onmouseout="this.style.color='var(--muted)'">← Back to login</button>
    </div>
  </div>
</div>
...
<nav class="sidebar">
  <div class="logo">
    <img id="sidebar-logo" src="https://i.ibb.co/RTmtK8sm/subloop.png" alt="Subloop" style="width:190px;height:auto;display:block;">
  </div>
...
<div id="page-dashboard" class="page active">
  ...
  <div class="stats-grid" id="stats-grid">
    ...
    <div class="stat-card"><div class="stat-label">Failed</div><div class="stat-value" id="stat-failed" style="color:var(--red)">—</div><div class="stat-sub" id="stat-dunning-sub">failed payments</div></div>
  ...
  <div class="card" style="overflow:visible;">
    <div style="padding:14px 18px 12px;display:flex;align-items:flex-start;justify-content:space-between;gap:12px;flex-wrap:wrap;">
      <div>
        <div style="font-size:13px;font-weight:600;">Revenue Overview</div>
        <div style="font-size:11px;color:var(--muted);font-family:var(--mono);margin-top:2px;">Track revenue by selected period</div>
      </div>
...
<div id="page-subscriptions" class="page">
  <div class="page-header"><div><h2>Subscriptions</h2><p>All rebilling schedules</p></div></div>
  <div class="search-bar"><input type="text" id="sub-search" placeholder="Search by name or email..." oninput="filterSubscriptions()"></div>
  <div class="card"><div class="scrollable"><table>
    <thead><tr><th>Customer</th><th>Amount</th><th>Interval</th><th>Next billing</th><th>Status</th><th>Actions</th></tr></thead>
    <tbody id="subscriptions-table"><tr><td colspan="6" class="empty">Loading...</td></tr></tbody>
  </table></div></div>
</div>
...
<div id="page-payments" class="page">
  ...
  <div class="card"><div class="scrollable"><table>
    <thead><tr><th>Customer</th><th>Amount</th><th>Status</th><th>Date</th><th>Reference</th><th>Action</th></tr></thead>
...
<div id="page-settings" class="page">
  ...
  <div class="card" style="max-width:600px;">
    ...
    <div style="padding:4px 18px 18px;">
      <p style="font-size:12px;color:var(--muted);font-family:var(--mono);margin:10px 0 16px;">When a rebill fails, automatically retry on a schedule before cancelling the subscription.</p>
  ...
  <div class="card" style="max-width:600px;">
    ...
    <div style="padding:4px 18px 18px;">
      <div class="setting-row">
        <div class="setting-info"><div class="setting-label">Auto-Resume Paused Subscriptions</div><div class="setting-desc">Automatically resume subscriptions when resume date is reached</div></div>
      ...
      <div class="setting-row">
        <div class="setting-info"><div class="setting-label">Proration</div><div class="setting-desc">Charge the difference immediately when upgrading a subscription mid-cycle</div></div>
  ...
  <div class="card" style="max-width:600px;">
    <div class="card-header"><h3>Analytics & Logs</h3></div>
    <div style="padding:4px 18px 18px;">
      <div class="setting-row">
        <div class="setting-info"><div class="setting-label">Webhook Logs</div><div class="setting-desc">Record all incoming webhook events for debugging</div></div>
...
<div id="page-security" class="page">
  ...
  <div class="card" style="max-width:600px;">
    ...
    <div style="padding:18px;">
      <div id="2fa-status-wrap">
        <div style="display:flex;align-items:center;gap:14px;margin-bottom:16px;">
          <div style="flex:1;">
            <div style="font-size:14px;font-weight:500;margin-bottom:3px;">Authenticator App</div>
            <div style="font-size:12px;color:var(--muted);font-family:var(--mono);">Use Google Authenticator, Authy, or any TOTP app</div>
          </div>
          <span id="2fa-badge" class="badge badge-red">disabled</span>
        </div>
        <div id="2fa-disabled-view">
          <p style="font-size:13px;color:var(--muted);margin-bottom:16px;">When enabled, you'll need to enter a 6-digit code from your authenticator app every time you log in.</p>
          <button class="btn btn-primary" onclick="setup2FA()">Enable 2FA</button>
        </div>
        <div id="2fa-setup-view" style="display:none;">
          <div style="text-align:center;margin-bottom:16px;">
            <img id="2fa-qr" src="" style="width:180px;height:180px;border-radius:10px;border:2px solid var(--border2);">
          </div>
          <p style="font-size:12px;color:var(--muted);font-family:var(--mono);text-align:center;margin-bottom:8px;">Scan with your authenticator app, then enter the 6-digit code below to confirm</p>
          <div style="display:flex;gap:8px;">
            <input type="text" id="2fa-verify-code" placeholder="000000" maxlength="6" style="text-align:center;font-size:18px;letter-spacing:6px;font-family:var(--mono);" oninput="this.value=this.value.replace(/\D/g,'')">
            <button class="btn btn-primary" onclick="confirm2FA()">Confirm</button>
          </div>
  ...
  <div class="card" style="max-width:600px;">
    ...
    <div style="padding:4px 18px 18px;">
      <div class="setting-row">
        <div class="setting-info"><div class="setting-label">Max Login Attempts</div><div class="setting-desc">Lock account after this many failed attempts</div></div>
        <input type="number" id="setting-max-attempts" style="width:70px;" min="3" max="20" onblur="saveSetting('max_login_attempts',this.value)">
      </div>
      <div class="setting-row">
        <div class="setting-info"><div class="setting-label">Lockout Duration (minutes)</div><div class="setting-desc">How long to block login after too many failures</div></div>
        <input type="number" id="setting-lockout-mins" style="width:70px;" min="5" max="60" onblur="saveSetting('lockout_minutes',this.value)">
      </div>
      <div class="setting-row">
        <div class="setting-info"><div class="setting-label">Session Timeout (minutes)</div><div class="setting-desc">Auto-logout after this period of inactivity</div></div>
        <input type="number" id="setting-session-timeout" style="width:70px;" min="30" max="1440" onblur="saveSetting('session_timeout',this.value)">
      </div>
    </div>
  </div>
...
<div class="modal-bg" id="add-account-modal">
  <div class="modal">
    <h3>Add Stripe account</h3><p>Connect a new Stripe account to manage multiple businesses</p>
    <div class="form-group"><label>Account name</label><input type="text" id="acc-name" placeholder="My Store / Client A"></div>
    <div class="form-group"><label>Secret key (sk_live_... or sk_test_...)</label><input type="password" id="acc-key" placeholder="sk_live_..."></div>
...
<div class="modal-bg" id="pause-modal">
  <div class="modal">
    <h3>Pause subscription</h3><p>Set an optional date to auto-resume this subscription</p>
...
<script>
let currentLink = null, allCustomers = [], allSubscriptions = [], appSettings = {};

// ── Theme ─────────────────────────────────────────────────────────────────────
function toggleTheme() {
  const isLight = document.documentElement.classList.toggle('light');
  localStorage.setItem('subloop_theme', isLight ? 'light' : 'dark');
  document.getElementById('theme-label').textContent = isLight ? 'Light mode' : 'Dark mode';
  updateLogos(isLight);
}
function updateLogos(isLight) {
  const src = isLight ? 'https://i.ibb.co/Q7YPRnvX/subloop-2.png' : 'https://i.ibb.co/RTmtK8sm/subloop.png';
  const s = document.getElementById('sidebar-logo'), l = document.getElementById('login-logo'), t = document.getElementById('twofa-logo');
  if (s) s.src = src; if (l) l.src = src; if (t) t.src = src;
}
function initTheme() {
  const isLight = localStorage.getItem('subloop_theme') === 'light';
  if (isLight) { document.documentElement.classList.add('light'); const l = document.getElementById('theme-label'); if (l) l.textContent = 'Light mode'; }
  updateLogos(isLight);
}
initTheme();

// ── Auth ──────────────────────────────────────────────────────────────────────
// doLogin defined in security section below
if (sessionStorage.getItem('subloop_auth') === '1') {
  document.getElementById('login-screen').classList.add('hidden');
  document.getElementById('app-screen').classList.remove('hidden');
  initApp();
}
// Also remove the old doLogin that was defined before
...
function showPage(name, el) {
  ...
  const loaders = { dashboard: ()=>{loadStats();loadUpcoming();loadRecentPayments();loadChart();}, activity: loadActivity, customers: loadCustomers, subscriptions: loadSubscriptions, payments: loadPayments, links: loadAccounts, settings: loadSettings, accounts: loadAccounts, webhooks: loadWebhooks, security: loadSecurity };
...
function cardBadge(brand, last4) {
  ...
  const src = icons[b];
  const imgOrFallback = src
    ? '<img src="'+src+'" class="payment-img" alt="'+b+'">'
    : '<div class="payment-badge-fallback">CARD</div>';
  const last4str = last4 ? '<span class="payment-dots">••••</span><span class="payment-last4">'+last4+'</span>' : '';
  return '<div class="payment-card">'+imgOrFallback+last4str+'</div>';
}

function initApp() { loadStats(); loadUpcoming(); loadRecentPayments(); loadChart(); }
...
async function loadStats() {
  try {
    ...
    if (s.dunning_subscriptions > 0) document.getElementById('stat-dunning-sub').textContent = s.dunning_subscriptions+' in dunning';
...
async function loadChart() {
  try {
    ...
    let filtered, periodLabel, bestLabel;
    if (currentRevPeriod === 'today') {
      const today = now.toISOString().split('T')[0];
      filtered = data.filter(d => d.day && d.day.startsWith(today));
      periodLabel = 'Today';
      bestLabel = 'Best Period';
    } else if (currentRevPeriod === 'yesterday') {
      const yest = new Date(now); yest.setDate(yest.getDate()-1);
      const yestStr = yest.toISOString().split('T')[0];
      filtered = data.filter(d => d.day && d.day.startsWith(yestStr));
      periodLabel = 'Yesterday';
      bestLabel = 'Best Period';
    } else if (currentRevPeriod === 'week') {
      const weekAgo = new Date(now); weekAgo.setDate(weekAgo.getDate()-7);
      filtered = data.filter(d => d.day && new Date(d.day) >= weekAgo);
    ...
    const activeDays = filtered.filter(d=>parseInt(d.revenue)>0);
    const best = activeDays.length ? activeDays.reduce((a,b)=>(parseInt(b.revenue)||0)>=(parseInt(a.revenue)||0)?b:a) : null;
    const bestDay = best ? new Date(best.day).toLocaleDateString('en-US',{weekday:'short',month:'short',day:'numeric'}) : '—';

    const cards = [
      {label:'Total Revenue', value:fmt(total), sub:periodLabel},
      {label:'Payments', value:totalCount, sub:'successful'},
      {label:'Avg Payment', value:fmt(avg), sub:'per transaction'},
      {label:bestLabel, value:fmt(parseInt(best?.revenue)||0), sub:bestDay},
    ];
    ...
    cards.forEach(c => {
      html += '<div style="background:var(--surface2);border:1px solid var(--border);border-radius:10px;padding:12px 14px;">'+
        '<div style="font-size:10px;color:var(--muted);font-family:var(--mono);text-transform:uppercase;letter-spacing:0.5px;margin-bottom:5px;">'+c.label+'</div>'+
        '<div style="font-size:18px;font-weight:700;letter-spacing:-0.5px;margin-bottom:3px;">'+c.value+'</div>'+
        '<div style="font-size:11px;color:var(--muted);font-family:var(--mono);">'+c.sub+'</div></div>';
    });
    html += '</div>';
    wrap.innerHTML = html;
...
async function loadUpcoming() {
  try {
    ...
    tb.innerHTML = subs.slice(0,10).map(s=>'<tr>'+
      ...
      '<td class="mono">'+fmt(s.amount,s.currency)+'</td><td class="mono">'+fmtDate(s.next_billing_date)+'</td><td>'+badge(s.status)+'</td>'+
      '<td><div class="actions">'+
        (s.status==='active'?'<button class="btn btn-sm" onclick="chargeNow('+s.id+')">Charge</button>':'')+
        (s.status==='active'?'<button class="btn btn-sm" onclick="openPauseModal('+s.id+')">Pause</button>':'')+
        (s.status==='paused'?'<button class="btn btn-sm" onclick="setSubStatus('+s.id+',\'active\')">Resume</button>':'')+
      '</div></td></tr>').join('');
  } catch(e) {}
}
...
async function loadRecentPayments() {
  try {
    ...
    tb.innerHTML = pmts.slice(0,8).map(p=>'<tr>'+
      '<td><div class="customer-cell"><div class="avatar">'+initials(p.name)+'</div><div><div class="customer-name">'+(p.name||'—')+'</div><div class="customer-email">'+(p.email||'')+'</div></div></div></td>'+
      '<td class="mono">'+fmt(p.amount,p.currency)+'</td><td>'+badge(p.status)+'</td><td class="mono">'+fmtDate(p.created_at)+'</td></tr>').join('');
  } catch(e) {}
}
...
async function loadActivity() {
  try {
    const list = await api('/api/activity');
    const wrap = document.getElementById('activity-list');
    if (!list.length) { wrap.innerHTML='<div class="empty">No activity yet</div>'; return; }
    wrap.innerHTML = list.map(a => {
      const dotClass = a.type.includes('fail')||a.type.includes('cancel') ? 'failed' : a.type.includes('dun') ? 'dunning' : a.type.includes('resume') ? 'resume' : a.type.includes('retry') ? 'retry' : a.type.includes('prora') ? 'proration' : 'payment';
      return '<div class="activity-item"><div class="activity-dot '+dotClass+'" style="margin-top:4px;"></div><div style="flex:1;"><div style="font-size:13px;">'+a.description+'</div><div style="font-size:11px;color:var(--muted);font-family:var(--mono);margin-top:2px;">'+fmtTime(a.created_at)+'</div></div></div>';
    }).join('');
  } catch(e) {}
}
...
async function loadCustomers() {
  try { allCustomers = await api('/api/customers'); renderCustomers(allCustomers); }
  catch(e) { document.getElementById('customers-table').innerHTML='<tr><td colspan="7" class="empty">Error</td></tr>'; }
}
function renderCustomers(list) {
  ...
  tb.innerHTML = list.map(c=>'<tr>'+
    ...
    '<td style="font-size:12px;color:var(--muted);">'+(c.account_name||'—')+'</td>'+
    '<td>'+badge(c.status)+'</td>'+
    '<td style="font-size:12px;color:var(--muted);font-family:var(--mono);max-width:150px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">'+(c.note||'—')+'</td>'+
    '<td class="mono">'+fmt(c.total_paid||0)+'</td>'+
    '<td><div class="actions">'+
      '<button class="btn btn-sm" onclick="openNoteModal('+c.id+',\''+encodeURIComponent(c.note||'')+'\')">Note</button>'+
      '<button class="btn btn-sm" onclick="openPortal('+c.id+')">Card ↗</button>'+
      (c.status==='active'?'<button class="btn btn-sm btn-danger" onclick="setCustStatus('+c.id+',\'paused\')">Pause</button>':'')+
      (c.status==='paused'?'<button class="btn btn-sm" onclick="setCustStatus('+c.id+',\'active\')">Resume</button>':'')+
    '</div></td></tr>').join('');
}
function filterCustomers() {
  const q = document.getElementById('customer-search').value.toLowerCase();
  renderCustomers(allCustomers.filter(c=>(c.name||'').toLowerCase().includes(q)||(c.email||'').toLowerCase().includes(q)));
}

// ── Subscriptions ─────────────────────────────────────────────────────────────
async function loadSubscriptions() {
  try { allSubscriptions = await api('/api/subscriptions'); renderSubscriptions(allSubscriptions); }
  catch(e) { document.getElementById('subscriptions-table').innerHTML='<tr><td colspan="6" class="empty">Error</td></tr>'; }
}
function renderSubscriptions(list) {
  const tb = document.getElementById('subscriptions-table');
  if (!list.length) { tb.innerHTML='<tr><td colspan="6" class="empty">No subscriptions yet</td></tr>'; return; }
  tb.innerHTML = list.map(s=>'<tr>'+
    '<td><div class="customer-cell"><div class="avatar">'+initials(s.name)+'</div><div><div class="customer-name">'+(s.name||'—')+'</div><div class="customer-email">'+(s.email||'')+'</div></div></div></td>'+
    '<td class="mono" id="amount-cell-'+s.id+'">'+fmt(s.amount,s.currency)+'</td>'+
    '<td class="mono">'+(s.interval_days||30)+' days</td>'+
    '<td class="mono">'+fmtDate(s.next_billing_date)+(s.resume_date?'<br><span style="font-size:10px;color:var(--blue);">Resumes '+fmtDate(s.resume_date)+'</span>':'')+'</td>'+
    '<td>'+badge(s.status)+(s.dunning_count>0?'<br><span style="font-size:10px;color:var(--amber);">Retry #'+s.dunning_count+'</span>':'')+'</td>'+
    '<td><div class="actions">'+
      (s.status==='active'?'<button class="btn btn-sm" onclick="chargeNow('+s.id+')">Charge</button>':'')+
      '<button class="btn btn-sm" onclick="showEditAmount('+s.id+','+s.amount+')">Edit $</button>'+
      '<span id="edit-'+s.id+'"></span>'+
      (s.status==='active'?'<button class="btn btn-sm" onclick="openPauseModal('+s.id+')">Pause</button>':'')+
      (s.status==='paused'?'<button class="btn btn-sm" onclick="setSubStatus('+s.id+',\'active\')">Resume</button>':'')+
      (s.status==='dunning'?'<button class="btn btn-sm btn-success" onclick="chargeNow('+s.id+')">Retry now</button>':'')+
      '<button class="btn btn-sm btn-danger" onclick="setSubStatus('+s.id+',\'cancelled\')">Cancel</button>'+
    '</div></td></tr>').join('');
}
function filterSubscriptions() {
  const q = document.getElementById('sub-search').value.toLowerCase();
  renderSubscriptions(allSubscriptions.filter(s=>(s.name||'').toLowerCase().includes(q)||(s.email||'').toLowerCase().includes(q)));
}
function showEditAmount(subId, currentAmount) {
  const container = document.getElementById('edit-'+subId);
  if (!container) return;
  const dollars = (currentAmount/100).toFixed(2);
  const prorationEnabled = appSettings.proration_enabled === 'true';
  container.innerHTML = '<div style="display:inline-flex;align-items:center;gap:4px;">'+
    '<input type="number" id="new-amount-'+subId+'" value="'+dollars+'" step="0.01" min="0.50" style="width:75px;padding:3px 6px;font-size:12px;">'+
    (prorationEnabled?'<button class="btn btn-sm btn-primary" onclick="saveAmount('+subId+',true)">Save+Prorate</button>':'')+
    '<button class="btn btn-sm btn-primary" onclick="saveAmount('+subId+',false)">Save</button>'+
    '<button class="btn btn-sm" onclick="loadSubscriptions()">✕</button></div>';
  document.getElementById('new-amount-'+subId).focus();
}
async function saveAmount(subId, prorate) {
  const input = document.getElementById('new-amount-'+subId);
  if (!input) return;
  const newCents = Math.round(parseFloat(input.value)*100);
  if (isNaN(newCents)||newCents<50) { toast('Minimum $0.50','error'); return; }
  const r = await api('/api/subscriptions/'+subId+'/amount',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({amount:newCents,prorate})});
  if (r.success) { toast(prorate?'✓ Amount updated + prorated':'✓ Amount updated'); loadSubscriptions(); }
...
async function loadPayments() {
  try {
    ...
    if (!list.length) { tb.innerHTML='<tr><td colspan="6" class="empty">No payments yet</td></tr>'; return; }
    tb.innerHTML = list.map(p=>'<tr>'+
      '<td><div class="customer-cell"><div class="avatar">'+initials(p.name)+'</div><div><div class="customer-name">'+(p.name||'—')+'</div><div class="customer-email">'+(p.email||'')+'</div></div></div></td>'+
      '<td class="mono">'+fmt(p.amount,p.currency)+'</td><td>'+badge(p.status)+'</td><td class="mono">'+fmtDate(p.created_at)+'</td>'+
      '<td class="mono" style="font-size:11px;color:var(--muted);">'+(p.stripe_payment_intent?p.stripe_payment_intent.slice(0,22)+'…':'—')+'</td>'+
      '<td>'+(p.status==='failed'?'<button class="btn btn-sm btn-success" onclick="retryPayment('+p.id+')">Retry</button>':'')+'</td></tr>').join('');
  } catch(e) {}
}
async function retryPayment(id) {
  toast('Retrying…');
  try {
    const r = await api('/api/payments/'+id+'/retry',{method:'POST'});
    if (r.success) { toast('✓ Payment succeeded'); loadPayments(); loadStats(); }
...
async function loadWebhooks() {
  try {
    ...
    tb.innerHTML = list.map(w=>'<tr>'+
      '<td class="mono" style="font-size:12px;">'+w.event_type+'</td>'+
      '<td style="font-size:12px;color:var(--muted);">'+(w.account_name||'—')+'</td>'+
      '<td>'+badge(w.status||'ok')+'</td>'+
      '<td class="mono" style="font-size:11px;color:var(--muted);">'+fmtTime(w.created_at)+'</td></tr>').join('');
  } catch(e) {}
}
...
async function saveSetting(key, value) {
  await api('/api/settings',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({key,value:String(value)})});
  appSettings[key] = String(value);
  toast('✓ Setting saved');
}

// ── Actions ───────────────────────────────────────────────────────────────────
async function chargeNow(subId) {
  toast('Charging…');
  try {
    const r = await api('/api/subscriptions/'+subId+'/charge',{method:'POST'});
    if (r.success) { toast('✓ Payment succeeded'); loadUpcoming(); loadStats(); loadActivity(); }
    else toast('✗ '+(r.error||'Failed'),'error');
  } catch(e) { toast('Error','error'); }
}
...
function openPauseModal(subId) {
  ...
  document.getElementById('pause-resume-date').value = '';
  openModal('pause-modal');
}
async function confirmPause() {
  const id = document.getElementById('pause-sub-id').value;
  const resumeDate = document.getElementById('pause-resume-date').value;
  await api('/api/subscriptions/'+id+'/status',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({status:'paused',resume_date:resumeDate||null})});
  toast('Subscription paused'+(resumeDate?' — resumes '+fmtDate(resumeDate):''));
  closeModal('pause-modal'); loadUpcoming(); loadSubscriptions();
}
async function setSubStatus(id, status) {
  await api('/api/subscriptions/'+id+'/status',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({status})});
  toast('Subscription '+status); loadUpcoming(); loadSubscriptions(); loadStats();
}
async function setCustStatus(id, status) {
  await api('/api/customers/'+id+'/status',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({status})});
  toast('Customer '+status); loadCustomers();
}
async function openPortal(customerId) {
  try { const r = await api('/api/customers/'+customerId+'/portal',{method:'POST'}); if (r.url) window.open(r.url,'_blank'); else toast('Could not open portal','error'); }
  catch(e) { toast('Error','error'); }
}
async function runRebills() {
  toast('Running rebill job…');
  try {
    const r = await api('/api/run-rebills',{method:'POST'});
    if (r.success) { toast('✓ Done: '+r.succeeded+' charged, '+r.failed+' failed'+(r.resumed>0?', '+r.resumed+' resumed':'')); loadStats(); loadUpcoming(); loadActivity(); }
  } catch(e) { toast('Error','error'); }
}
...
async function addCustomer() {
  ...
  try { const r = await api('/api/customers',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}); if (r.success) { toast('✓ Customer added'); closeModal('add-customer-modal'); loadCustomers(); } else toast('Error: '+(r.error||'Failed'),'error'); }
  catch(e) { toast('Error','error'); }
}
...
async function saveNote() {
  const id=document.getElementById('note-customer-id').value, note=document.getElementById('note-text').value;
  await api('/api/customers/'+id+'/note',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({note})});
  toast('✓ Note saved'); closeModal('note-modal'); loadCustomers();
}
...
async function createPaymentLink() {
  ...
  try {
    const r = await api('/api/payment-links',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:document.getElementById('link-name').value,amount:Math.round(parseFloat(document.getElementById('link-amount').value)*100),currency:document.getElementById('link-currency').value,interval_days:parseInt(document.getElementById('link-interval').value),stripe_account_id:document.getElementById('link-account').value||null})});
    if (r.url) { currentLink=r.url; document.getElementById('link-url').textContent=r.url; document.getElementById('link-result').classList.add('visible'); toast('✓ Link created'); }
...
async function loadAccounts() {
  try {
    ...
    if (tb) {
      if (!list.length) tb.innerHTML='<tr><td colspan="4" class="empty">No accounts yet</td></tr>';
      else tb.innerHTML = list.map(a=>'<tr><td><strong>'+a.name+'</strong></td><td class="mono" style="font-size:12px;">'+(a.key_preview||'—')+'</td><td>'+(a.is_default?'<span class="badge badge-green">default</span>':'<span class="badge badge-blue">active</span>')+'</td><td><div class="actions">'+(!a.is_default?'<button class="btn btn-sm" onclick="setDefaultAccount('+a.id+')">Set default</button>':'')+'<button class="btn btn-sm btn-danger" onclick="deleteAccount('+a.id+')">Delete</button></div></td></tr>').join('');
    }
...
async function addAccount() {
  const name=document.getElementById('acc-name').value.trim(), key=document.getElementById('acc-key').value.trim(), webhook=document.getElementById('acc-webhook').value.trim();
  if (!name||!key) { toast('Name and key required','error'); return; }
  const r = await api('/api/stripe-accounts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,secret_key:key,webhook_secret:webhook})});
  if (r.success) { toast('✓ Account added'); closeModal('add-account-modal'); document.getElementById('acc-name').value=''; document.getElementById('acc-key').value=''; document.getElementById('acc-webhook').value=''; loadAccounts(); }
  else toast('Error: '+(r.error||'Failed'),'error');
}
...
// Pre-load settings for proration button
...
async function loadSecurity() {
  try {
    ...
    document.getElementById('setting-session-timeout').value = s.session_timeout || '480';
    // Load login history
    ...
    tb.innerHTML = history.map(h =>
      '<tr><td class="mono" style="font-size:12px;">'+(h.ip||'unknown')+'</td>'+
      '<td>'+(h.success?'<span class="badge badge-green">success</span>':'<span class="badge badge-red">failed</span>')+'</td>'+
      '<td class="mono" style="font-size:11px;color:var(--muted);">'+fmtTime(h.created_at)+'</td></tr>'
    ).join('');
  } catch(e) {}
}
...
function update2FAView(enabled) {
  ...
  const disabled = document.getElementById('2fa-disabled-view');
  const setup = document.getElementById('2fa-setup-view');
  const enabledView = document.getElementById('2fa-enabled-view');
  badge.className = 'badge ' + (enabled ? 'badge-green' : 'badge-red');
  badge.textContent = enabled ? 'enabled' : 'disabled';
  disabled.style.display = enabled ? 'none' : 'block';
...
async function setup2FA() {
  try {
    const r = await api('/api/security/2fa/setup', { method: 'POST' });
    document.getElementById('2fa-qr').src = r.qrCode;
    document.getElementById('2fa-disabled-view').style.display = 'none';
    document.getElementById('2fa-setup-view').style.display = 'block';
  } catch(e) { toast('Error setting up 2FA', 'error'); }
}

async function confirm2FA() {
  const token = document.getElementById('2fa-verify-code').value;
  if (token.length !== 6) { toast('Enter 6-digit code', 'error'); return; }
  try {
    const r = await api('/api/security/2fa/verify', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ token }) });
    if (r.success) { toast('✓ 2FA enabled!'); update2FAView(true); }
    else toast('Invalid code, try again', 'error');
  } catch(e) { toast('Error', 'error'); }
}
...
async function disable2FA() {
  ...
  toast('2FA disabled'); update2FAView(false);
}
...
async function doLogin() {
  // Check lockout first
  try {
    const lockout = await api('/api/security/lockout-check');
    if (lockout.locked) {
      const msg = document.getElementById('lockout-msg');
      msg.style.display = 'block';
      msg.textContent = 'Too many failed attempts. Try again in ' + lockout.lockoutMins + ' minutes.';
      return;
    }
  } catch(e) {}
  ...
  if (user === 'Tharos333' && pass === 'IssoMoussa544@###') {
    // Check if 2FA is enabled
    try {
      const status = await api('/api/security/2fa/status');
      if (status.enabled) {
        // Show premium 2FA screen
        document.getElementById('login-screen').style.display = 'none';
        const tfaScreen = document.getElementById('twofa-screen');
        tfaScreen.style.display = 'flex';
        // Update logo based on theme
        const isLight = document.documentElement.classList.contains('light');
        document.getElementById('twofa-logo').src = isLight ? 'https://i.ibb.co/Q7YPRnvX/subloop-2.png' : 'https://i.ibb.co/RTmtK8sm/subloop.png';
        setTimeout(()=>document.getElementById('otp-0').focus(), 100);
        await api('/api/security/login-attempt', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ success: false }) }); // don't count as success until 2FA done
        return;
      }
    } catch(e) {}
    // No 2FA — log in directly
    await api('/api/security/login-attempt', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ success: true }) }).catch(()=>{});
    completeLogin();
  } else {
    document.getElementById('login-error').classList.add('visible');
    document.getElementById('login-pass').value = '';
    document.getElementById('login-pass').focus();
    await api('/api/security/login-attempt', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ success: false }) }).catch(()=>{});
  }
}
...
function completeLogin() {
  sessionStorage.setItem('subloop_auth', '1');
  sessionStorage.setItem('subloop_login_time', Date.now().toString());
  document.getElementById('login-screen').classList.add('hidden');
  document.getElementById('app-screen').classList.remove('hidden');
...
function checkSessionTimeout() {
  const loginTime = parseInt(sessionStorage.getItem('subloop_login_time') || '0');
  const timeoutMins = parseInt(appSettings.session_timeout || '480');
  if (loginTime && Date.now() - loginTime > timeoutMins * 60 * 1000) {
    sessionStorage.removeItem('subloop_auth');
    sessionStorage.removeItem('subloop_login_time');
