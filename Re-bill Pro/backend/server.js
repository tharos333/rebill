...
app.post('/api/payment-links', async (req, res) => {
  try {
    ...
    const acc = account_id ? await stripeAccounts.byId(account_id) : await stripeAccounts.default();
  ...
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      try {
        ...
        // Record payment if amount exists
        if (session.amount_total) {
          const custRow = await pool.query('SELECT id FROM customers WHERE stripe_customer_id=$1', [session.customer]);
          if (custRow.rows[0]) {
            await pool.query("INSERT INTO payments (customer_id,stripe_payment_intent,amount,currency,status) VALUES ($1,$2,$3,$4,'succeeded')",
              [custRow.rows[0].id, session.payment_intent, session.amount_total, session.currency||'usd']);
          }
        }
