...
async function init() {
  await pool.query(`
    ...
    CREATE TABLE IF NOT EXISTS shopify_orders (
      id SERIAL PRIMARY KEY,
      store_id INT REFERENCES shopify_stores(id) ON DELETE CASCADE,
    ...
    CREATE TABLE IF NOT EXISTS shopify_webhook_logs (
      id SERIAL PRIMARY KEY,
      store_id INT REFERENCES shopify_stores(id) ON DELETE SET NULL,
      topic TEXT,
...
const payments = {
  recent: async (limit=50) => { const r = await pool.query('SELECT p.*, c.email, c.name, COALESCE(p.card_brand,c.card_brand) AS card_brand, COALESCE(p.card_last4,c.card_last4) AS card_last4 FROM payments p JOIN customers c ON c.id=p.customer_id ORDER BY p.created_at DESC LIMIT $1', [limit]); return r.rows; },
...
const shopifyDb = {
  stores: async () => {
    const r = await pool.query("SELECT id, name, shop_domain, status, currency, last_sync_at, created_at, '••••••' as token_preview FROM shopify_stores ORDER BY created_at DESC, id DESC");
    return r.rows;
  },
  createStore: async ({ name, shop_domain, access_token }) => {
    const r = await pool.query(
      `INSERT INTO shopify_stores (name, shop_domain, access_token)
       VALUES ($1,$2,$3)
       ON CONFLICT (shop_domain) DO UPDATE SET name=EXCLUDED.name, access_token=EXCLUDED.access_token, status='active'
       RETURNING id, name, shop_domain, status, currency, last_sync_at, created_at`,
      [name, shop_domain, access_token]
    );
    return r.rows[0];
  },
  deleteStore: async (id) => { await pool.query('DELETE FROM shopify_stores WHERE id=$1', [id]); },
  getStore: async (id) => {
    const r = await pool.query('SELECT * FROM shopify_stores WHERE id=$1', [id]);
    return r.rows[0];
  },
  saveOrder: async (storeId, order) => {
    await pool.query(
      `INSERT INTO shopify_orders (store_id, shopify_order_id, order_name, customer_email, customer_name, total_price_cents, currency, financial_status, fulfillment_status, processed_at, created_at)
  ...
  saveCustomer: async (storeId, customer) => {
    if (!customer || !customer.id) return;
    const name = [customer.first_name, customer.last_name].filter(Boolean).join(' ') || customer.name || customer.email || ('Customer '+customer.id);
    const totalSpent = Math.round(parseFloat(customer.total_spent || customer.total_spent_set?.shop_money?.amount || 0) * 100);
    await pool.query(
      `INSERT INTO shopify_customers (store_id, shopify_customer_id, email, name, orders_count, total_spent_cents, currency, updated_at)
       VALUES ($1,$2,$3,$4,$5,$6,$7,NOW())
       ON CONFLICT (store_id, shopify_customer_id) DO UPDATE SET
        email=EXCLUDED.email,
        name=EXCLUDED.name,
        orders_count=GREATEST(shopify_customers.orders_count, EXCLUDED.orders_count),
        total_spent_cents=GREATEST(shopify_customers.total_spent_cents, EXCLUDED.total_spent_cents),
        currency=EXCLUDED.currency,
        updated_at=NOW()`,
      [storeId, String(customer.id), customer.email || null, name, customer.orders_count || 0, totalSpent || 0, (customer.currency || 'USD').toUpperCase()]
    );
  },
  markSynced: async (storeId) => { await pool.query('UPDATE shopify_stores SET last_sync_at=NOW() WHERE id=$1', [storeId]); },
  orders: async (limit=100) => {
    const r = await pool.query(`SELECT o.*, s.name as store_name, s.shop_domain FROM shopify_orders o LEFT JOIN shopify_stores s ON s.id=o.store_id ORDER BY COALESCE(o.processed_at,o.created_at) DESC, o.id DESC LIMIT $1`, [limit]);
    return r.rows;
  },
  customers: async (limit=100) => {
    const r = await pool.query(`SELECT c.*, s.name as store_name, s.shop_domain FROM shopify_customers c LEFT JOIN shopify_stores s ON s.id=c.store_id ORDER BY c.updated_at DESC, c.id DESC LIMIT $1`, [limit]);
    return r.rows;
  },
  summary: async () => {
    const r = await pool.query(`
      SELECT
        (SELECT COUNT(*) FROM shopify_stores) as stores,
        (SELECT COUNT(*) FROM shopify_orders) as orders,
        (SELECT COALESCE(SUM(total_price_cents),0) FROM shopify_orders WHERE financial_status IN ('paid','partially_paid')) as revenue,
        (SELECT COUNT(*) FROM shopify_customers) as customers
    `);
    return r.rows[0];
  },
  logWebhook: async (storeId, topic, status='ok', error=null) => {
    await pool.query('INSERT INTO shopify_webhook_logs (store_id, topic, status, error) VALUES ($1,$2,$3,$4)', [storeId||null, topic||null, status, error||null]).catch(()=>{});
  },
  webhookLogs: async (limit=50) => {
    const r = await pool.query('SELECT l.*, s.name as store_name FROM shopify_webhook_logs l LEFT JOIN shopify_stores s ON s.id=l.store_id ORDER BY l.created_at DESC LIMIT $1', [limit]);
    return r.rows;
  }
};

module.exports = { init, pool, settingsDb, stripeAccounts, customers, subscriptions, payments, activityLog, webhookLogs, security, adminUsers, shopifyDb };
