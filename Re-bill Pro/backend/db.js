...
async function init() {
  ...
  const defaults = {
    ...
    churn_alert_enabled: 'false',
    bulk_actions_enabled: 'true',
    scheduled_billing_enabled: 'true',
...
const stripeAccounts = {
  ...
  create: async (data) => {
    ...
    const r = await pool.query('INSERT INTO stripe_accounts (name,secret_key,webhook_secret,is_default) VALUES ($1,$2,$3,$4) RETURNING id',
...
const subscriptions = {
  ...
  dunningDue: async () => {
    const r = await pool.query(`
      ...
      FROM subscriptions s JOIN customers c ON c.id=s.customer_id
      LEFT JOIN stripe_accounts sa ON sa.id=c.stripe_account_id
      WHERE s.status='dunning' AND c.status='active'
  ...
  resumeDue: async () => {
    const r = await pool.query(`
      ...
      WHERE status='paused' AND resume_date IS NOT NULL AND resume_date <= CURRENT_DATE
    `);
    return r.rows;
  },
  create: async (data) => {
    await pool.query('INSERT INTO subscriptions (customer_id,amount,currency,interval_days,next_billing_date) VALUES ($1,$2,$3,$4,$5)',
      [data.customer_id, data.amount, data.currency, data.interval_days, data.next_billing_date]);
  },
  advanceBillingDate: async (id, days) => {
    await pool.query("UPDATE subscriptions SET next_billing_date=next_billing_date+$1*INTERVAL '1 day', dunning_count=0, last_failed_at=NULL WHERE id=$2", [days, id]);
  },
  ...
  markDunning: async (id, retryDate) => {
    await pool.query('UPDATE subscriptions SET status=\'dunning\', next_billing_date=$1, dunning_count=dunning_count+1, last_failed_at=NOW() WHERE id=$2', [retryDate, id]);
  },
};
...
const payments = {
  recent: async (limit=50) => {
    const r = await pool.query('SELECT p.*, c.email, c.name FROM payments p JOIN customers c ON c.id=p.customer_id ORDER BY p.created_at DESC LIMIT $1', [limit]);
    return r.rows;
  },
  ...
  stats: async () => {
    const r = await pool.query(`
      SELECT
        ...
        COUNT(CASE WHEN status='succeeded' AND created_at >= NOW()-INTERVAL '30 days' THEN 1 END) as count_30d,
        COALESCE(SUM(CASE WHEN status='succeeded' AND created_at >= NOW()-INTERVAL '30 days' THEN amount ELSE 0 END),0) as revenue_30d
      FROM payments
    `);
    return r.rows[0];
  },
  insert: async (data) => {
    await pool.query('INSERT INTO payments (customer_id,subscription_id,stripe_payment_intent,amount,currency,status,failure_reason) VALUES ($1,$2,$3,$4,$5,$6,$7)',
...
// ── Activity Log ──────────────────────────────────────────────────────────────
const activityLog = {
  add: async (type, description, customer_id=null, amount=null) => {
    await pool.query('INSERT INTO activity_log (type,description,customer_id,amount) VALUES ($1,$2,$3,$4)',
      [type, description, customer_id, amount]).catch(()=>{});
  },
  recent: async (limit=50) => {
    const r = await pool.query('SELECT a.*, c.name as customer_name, c.email FROM activity_log a LEFT JOIN customers c ON c.id=a.customer_id ORDER BY a.created_at DESC LIMIT $1', [limit]);
    return r.rows;
  },
};
...
const webhookLogs = {
  add: async (data) => {
    await pool.query('INSERT INTO webhook_logs (event_type,account_name,status,error) VALUES ($1,$2,$3,$4)',
      [data.event_type, data.account_name||null, data.status||'ok', data.error||null]).catch(()=>{});
  },
  recent: async (limit=50) => {
    const r = await pool.query('SELECT * FROM webhook_logs ORDER BY created_at DESC LIMIT $1', [limit]);
    return r.rows;
  },
};
