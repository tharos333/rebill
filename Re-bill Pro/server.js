...
app.get('*', async (req, res) => {
  try {
    ...
    if (req.path === '/admin' || req.path.startsWith('/admin/')) {
      ...
      res.set('Cache-Control', 'no-store, no-cache, must-revalidate, proxy-revalidate'); res.set('Pragma', 'no-cache'); res.set('Expires', '0'); res.set('X-Subloop-Admin-Build', '20260827-signout-real-fix-4'); return res.sendFile(path.join(__dirname, 'admin.html'));
    }
